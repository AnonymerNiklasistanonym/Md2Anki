...
def download_script_files(dir_path: str = os.path.join(CURRENT_DIR, "temp"), skip_download_if_existing: bool = True):
    if not os.path.isdir(dir_path):
        os.mkdir(dir_path)
    if not os.path.isfile(os.path.join(dir_path, HLJS_CSS_FILE_NAME)) or not skip_download_if_existing:
        urllib.request.urlretrieve(HLJS_CSS_URL, os.path.join(dir_path, HLJS_CSS_FILE_NAME))
...
def create_katex_highlightjs_anki_deck_model(dir_path: str = os.path.join(CURRENT_DIR, "temp"),
                                             ...
                                             online: bool = True, debug=False) -> AnkiModel:
    ...
    if online:
        ...
        highlightjs_js_template_code += f'<link rel="stylesheet" href="{HLJS_CSS_URL}" crossorigin="anonymous">\n' \
                                        f'<script src="{HLJS_URL}" crossorigin="anonymous"></script>\n'

        if debug:
            print(f"katex_js_template_code[online]: {katex_js_template_code}")
            print(f"highlightjs_js_template_code[online]: {highlightjs_js_template_code}")
    else:
        ...
        with open(os.path.join(dir_path, HLJS_CSS_FILE_NAME), 'r') as highlightjs_css_file:
...
if __name__ == '__main__':
    ...
    anki_deck.model = create_katex_highlightjs_anki_deck_model(online=True, debug=debug_flag_found)
